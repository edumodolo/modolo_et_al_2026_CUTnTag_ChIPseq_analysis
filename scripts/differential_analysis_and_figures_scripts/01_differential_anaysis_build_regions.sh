...
#   deduplicated BAMs with deepTools multiBamSummary (see PART 4 for the
#   measured justification of the counting flags).
#
...
# that the peak-overlap validation (03) and the control figures (05) can ask
...
#
# ONE FRAGMENT, ONE COUNT. Measured directly on these libraries rather than
# assumed (deepTools 3.5.6, samtools 1.24; 4000 x 5 kb bins on chr1:0-20Mb of
# CnT_H3K4me3_K562_KayaOkur_2019_rep2):
#
#   samtools read1 of proper pairs (= fragments) :  50,696
#   --extendReads                                : 109,215   ratio 2.15
#   --extendReads --samFlagInclude 64            :  54,608   ratio 1.08
#
...
# once. The per-bin ratio was 0.5000 in every bin with no bin losing all
# signal, i.e. a uniform halving rather than a subset of fragments being
# dropped. The residual 8% above the samtools count is bin-edge overlap: a
# fragment spanning a boundary is counted in both bins. That the excess is 8%
# rather than ~1% also confirms reads are genuinely extended to the fragment
# span and not merely counted at read length.
#
# The filter is applied to PAIRED-END BATCHES ONLY. Single-end reads never set
# the 0x40 bit -- measured as exactly 0 first-in-pair reads in
# chip_H3K4me3_K562_Bernstein_2017_rep1 -- so applying it there would return
# zero counts for every bin.
#
# Re-measure with test_multibamsummary_counting.sh if the deepTools version
# changes; this behaviour is version-dependent.
